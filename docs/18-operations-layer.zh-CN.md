...
节点详情的运维页提供“服务器到本机延迟”探测。来源自动绑定当前节点 Agent，目标默认
为 `42.49.64.154`，也可改为其他 IPv4/IPv6 地址。Agent 只接收经过控制面规范化的 IP，
受限 Helper 固定执行 4 次 ICMP 请求和 2 秒单次等待，不接受主机名、命令参数或 shell。

普通节点归档会等待 Agent 确认最后一次用户清理。失联节点若已经停用且不存在未归档用户
分配，控制台会显示第二次风险确认并允许强制归档；这只移除控制面记录，不能证明失联服务
器的数据面已经删除旧凭据。仍在使用或仍有用户分配的节点不能强制归档。
